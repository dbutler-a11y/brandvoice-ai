...
export default function StrategyPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setIsEditing(false)
      setSuccessMessage('Strategy notes saved successfully!')
  ...
  const renderMarkdown = (md: string) => {
    const html = md
      ...
      .replace(/```[\s\S]*?```/gim, (match) => {
        ...
        return `<pre class="bg-gray-100 p-4 rounded-lg overflow-x-auto my-4 text-sm"><code class="text-gray-800">${code}</code></pre>`
      })
      // Inline code
      .replace(/`([^`]+)`/gim, '<code class="bg-gray-100 px-1 py-0.5 rounded text-sm text-gray-800">$1</code>')
      // Blockquotes
      .replace(/^> (.*$)/gim, '<blockquote class="border-l-4 border-purple-500 pl-4 py-1 my-2 text-gray-600 italic">$1</blockquote>')
      // Horizontal rules
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <div className="text-gray-500">Loading strategy notes...</div>
      </div>
    )
  }
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">Pricing, positioning, and growth strategy notes</p>
        </div>
        ...
        {isEditing ? (
          <div className="p-6">
            ...
            <textarea
              ...
              className="w-full h-[70vh] px-4 py-3 border border-gray-300 rounded-md focus:ring-purple-500 focus:border-purple-500 font-mono text-sm text-gray-900"
              placeholder="Write your strategy notes in Markdown..."
            />
            <p className="mt-2 text-sm text-gray-500">
              Supports Markdown: # Headers, **bold**, *italic*, `code`, - [ ] checkboxes, --- dividers, tables
            </p>
          </div>
        ) : (
          <div
